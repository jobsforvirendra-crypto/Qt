...
Item {
    ...
    property int fuelLavel : controller ? controller.fuel : 0
    width: 150
    height: 150
    //property int fuelLavel: fuelCtrlId.fuel
    property color gaugeColor: {
        //  console.log("fuelLavel: ",fuelLavel)
        if (fuelLavel < 50) return "#F44336"
        else if (fuelLavel >= 50 && fuelLavel <80 ) return "#FFC107"
        else return "#4CAF50"
    }
    ...
    Rectangle{
        ...
        transform: Rotation{
            ...
            angle: -60 + (gaugeId.fuelLavel * 1.2 )
            Behavior on angle {
